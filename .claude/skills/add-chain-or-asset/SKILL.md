...
description: Add a new chain or asset to chainflip-product-toolkit (the source-of-truth packages the rest of the webstack depends on). Use when adding a chain/asset here, or when the chainflip-add-chain-or-asset orchestrator delegates the product-toolkit step. Covers enums/constants, RPC & redis parsers, the address codec (util for EVM-compatible, package for non-EVM), processor codegen, and publishing @chainflip/* packages.
---
