...
import { hexToTronAddress, isValidTronAddress, tronAddressToHex } from '../tron';
...
describe('tron', () => {
  ...
  describe('tron address to hex', () => {
    it.each([['TR7NHqjeKQxGTCi8q8ZY4pL8otSzgjLj6t', 'a614f803b6fd780986a42c78ec9c7f77e6ded13c']])(
      'tronAddressToHex(%s) === %s',
      (address, expected) => {
        expect(tronAddressToHex(address)).toBe(expected);
      },
    );
  });
});
